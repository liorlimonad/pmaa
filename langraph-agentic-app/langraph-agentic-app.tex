...
\begin{enumerate}
    \item \texttt{ingest\_event}
    \item \texttt{load\_issue\_context}
    \item \texttt{classify\_intent\_and\_state}
    \item \texttt{policy\_guard}
    \item \texttt{route\_to\_role\_agent}
    \item role-specific node:
    \begin{itemize}
        \item \texttt{issue\_reporter\_agent}, or
        \item \texttt{bot\_workflow\_agent}, or
        \item \texttt{implementation\_agent}, or
        \item \texttt{quality\_agent}, or
        \item \texttt{technical\_writer\_agent}
    \end{itemize}
    \item \texttt{human\_approval\_gate}
    \item \texttt{execute\_actions}
    \item \texttt{update\_memory\_and\_metrics}
    \item \texttt{close\_or\_wait}
\end{enumerate}

\subsection{Routable Roles}

In the current implementation, the effective routable roles are:
\begin{itemize}
    \item \texttt{issue\_reporter}
    \item \texttt{bot}
    \item \texttt{implementation}
    \item \texttt{quality}
    \item \texttt{technical\_writer}
\end{itemize}

There is also a partial \texttt{devops} hypothesis in the classifier, but it is not fully supported by the final dispatch layer. That means \texttt{devops} is conceptually present in classification logic but not fully implemented as a stable runtime route.
